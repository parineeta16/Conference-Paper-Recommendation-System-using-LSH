...
pickle_in = open("Abstract_Data_Prep.pickle","rb")
Data_Prep = pickle.load(pickle_in)

#Separate the text value as words in a list (tokenization)
for i in Data_Prep:
    Data_Prep[i] = re.sub("[^\w]", " ", Data_Prep[i]).split()

#print("Abstract of first Conference Paper:\n",Data_Prep[0])
#print("\nTotal elements in the dictionary =",len(Data_Prep))
...
for i in range(0,len(Data_Prep)): #len(Data_Prep)):
...
#print("Signature Matrix values for first 5 documents:")
#print(signatures[0],"\n",signatures[1],"\n",signatures[2],"\n",signatures[3],"\n",signatures[4])
...
while True:
    try:
        docid = int(input(
            "Please enter the document id you are interested in. The valid document ids are 0 - " + str(
                numDocs-1) + ": "))
    except ValueError:
...
print("Recommendations based on True Jaccard Similarity are:")
...
for i in range(0, neighbors):
    ...
    # Calculate the elapsed time (in seconds)
elapsed = (time.time() - t0)
print("Time required to calculate True Jaccard Similarity:",elapsed)
